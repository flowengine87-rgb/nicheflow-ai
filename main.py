# -*- coding: utf-8 -*-
"""
NicheFlow AI — main.py  FIXED v3
- /pipeline endpoint (React frontend calls this directly)
- /webhook/lemonsqueezy — auto-upgrades users to Pro after payment
...
LEMONSQUEEZY_SECRET = os.getenv("LEMONSQUEEZY_WEBHOOK_SECRET", "")  # Set in Railway env vars

# LemonSqueezy product IDs — set these after you get approved
LEMON_BASIC_VARIANT_ID = os.getenv("LEMON_BASIC_VARIANT_ID", "")   # Basic $30/mo variant ID
LEMON_PRO_VARIANT_ID   = os.getenv("LEMON_PRO_VARIANT_ID", "")     # Pro $40/mo variant ID

app = FastAPI(title="NicheFlow AI API", version="3.0.0")
...
def save_article_to_db(user_id, token, title, result, publish_status):
    data = {"user_id": user_id, "title": title, "post_url": result.get("post_url",""),
            "post_id": str(result.get("post_id","")), "featured_image_url": result.get("featured_image_url",""),
            "status": "published" if result.get("success") else "failed",
            "wp_status": publish_status, "error": result.get("error","")}
...
def upgrade_user_plan(user_email: str, plan: str):
    ...
    # Find user by email in auth.users (we check profiles table)
    resp = requests.get(
...
@app.get("/health")
def health():
    return {"status": "ok", "service": "NicheFlow AI", "version": "3.0.0"}
...
def pipeline_direct(body: PipelineRequest, authorization: str = Header(None)):
    if not body.gemini_key or not body.gemini_key.strip():
        raise HTTPException(status_code=400, detail="No AI key. Go to Settings → API Keys and save your Groq or Gemini key.")
    ...
    result = run_full_pipeline(
        title=body.title, gemini_key=body.gemini_key.strip(),
        goapi_key=(body.goapi_key or "").strip(), wp_url=body.wp_url.strip(),
        wp_password=body.wp_password.strip(), publish_status=body.publish_status or "publish",
        mj_template=(body.mj_template or "").strip(), custom_prompt=(body.custom_prompt or "").strip(),
        card_prompt=(body.card_prompt or "").strip(), show_card=body.show_card if body.show_card is not None else True,
        use_images=body.use_images or False, use_pollinations=body.use_pollinations or False,
        pollinations_prompt=(body.pollinations_prompt or "").strip(), category_ids=body.category_ids,
        max_links=body.max_links or 4, use_internal_links=body.use_internal_links if body.use_internal_links is not None else True,
    )
...
def generate(body: GenerateRequest, auth=Depends(get_current_user)):
    ...
    for title in body.titles:
        ...
        results.append({"title":title,"success":result.get("success"),"post_url":result.get("post_url",""),
                        "error":result.get("error",""),"article_id":article_id,"featured_image_url":result.get("featured_image_url","")})

        plan = get_user_plan(user_id, token)
        if plan == "pro" and cfg.get("auto_pin") and result.get("success") and cfg.get("pinterest_token") and cfg.get("pinterest_boards"):
            board_ids = [b.strip() for b in cfg["pinterest_boards"].split(",") if b.strip()]
            run_pinterest_bot(api_key=cfg["gemini_key"], access_token=cfg["pinterest_token"],
                articles=[{"title":title,"post_url":result.get("post_url",""),"featured_image_url":result.get("featured_image_url","")}],
                board_ids=board_ids, pinterest_prompt=cfg.get("pinterest_prompt",""), pin_delay_min=cfg.get("pin_delay_min",0))

        delay = body.delay_sec or cfg.get("delay_sec",10)
        if delay > 0 and title != body.titles[-1]: time.sleep(delay)
...
def get_history(auth=Depends(get_current_user)):
    user, token = auth
    resp = requests.get(f"{SUPABASE_URL}/rest/v1/articles?user_id=eq.{user['id']}&order=created_at.desc&limit=200",
        headers=supa_headers(token), timeout=10)
...
def delete_article(article_id: str, auth=Depends(get_current_user)):
    user, token = auth
    resp = requests.delete(f"{SUPABASE_URL}/rest/v1/articles?id=eq.{article_id}&user_id=eq.{user['id']}",
        headers=supa_headers(token), timeout=10)
...
def clear_history(auth=Depends(get_current_user)):
    user, token = auth
    resp = requests.delete(f"{SUPABASE_URL}/rest/v1/articles?user_id=eq.{user['id']}",
        headers=supa_headers(token), timeout=10)
    return {"success": resp.status_code in (200, 204)}
...
def pinterest_run(body: PinterestRunRequest, auth=Depends(get_current_user)):
    ...
    if body.article_ids: query += f"&id=in.({','.join(body.article_ids)})"
    art_resp = requests.get(query, headers=supa_headers(token), timeout=10)
    articles = art_resp.json() if art_resp.status_code == 200 else []
    if not articles: raise HTTPException(status_code=404, detail="No published articles found.")

    results = run_pinterest_bot(api_key=cfg["gemini_key"], access_token=cfg["pinterest_token"],
        articles=articles, board_ids=body.board_ids, pinterest_prompt=cfg.get("pinterest_prompt",""),
        pin_delay_min=cfg.get("pin_delay_min",0))

    for r in results:
        for br in r.get("boards",[]):
            requests.post(f"{SUPABASE_URL}/rest/v1/pinterest_pins", headers=supa_headers(token),
                json={"user_id":user["id"],"pin_title":r.get("pin_title",""),"pin_desc":r.get("pin_description",""),
                      "alt_text":r.get("alt_text",""),"board_ids":br.get("board_id",""),"pin_id":br.get("pin_id",""),
                      "status":"sent" if br.get("success") else "failed","error":br.get("error","")}, timeout=10)
...
def checkout_basic(auth=Depends(get_current_user)):
    """Returns the LemonSqueezy checkout URL for Basic plan."""
    user, token = auth
    email = user.get("email", "")
    # Embed user email as custom data so webhook can identify them
    base_url = os.getenv("LEMON_BASIC_URL", "")  # Set in Railway: your LemonSqueezy checkout URL
    if not base_url:
...
def checkout_pro(auth=Depends(get_current_user)):
    """Returns the LemonSqueezy checkout URL for Pro plan."""
    user, token = auth
    email = user.get("email", "")
    base_url = os.getenv("LEMON_PRO_URL", "")  # Set in Railway: your LemonSqueezy checkout URL
    if not base_url:
...
async def lemonsqueezy_webhook(request: Request):
    """
    LemonSqueezy sends events here after payment.
    Set this URL in LemonSqueezy dashboard → Webhooks:
    https://your-railway-url.up.railway.app/webhook/lemonsqueezy

    Events we handle:
    - order_created → upgrade user to correct plan
    - subscription_created → upgrade user
    - subscription_cancelled → downgrade to basic
    """
    ...
    # Get customer email — LemonSqueezy provides this in different places
    user_email = (
    ...
    # Get variant ID to determine which plan
    ...
    if event in ("order_created", "subscription_created", "subscription_payment_success"):
        ...
        if variant_id == str(LEMON_PRO_VARIANT_ID) or "pro" in str(attributes.get("product_name","")).lower():
            ...
            plan = "basic"  # Default to basic for any other purchase
...
def get_profile(auth=Depends(get_current_user)):
    user, token = auth
    resp = requests.get(f"{SUPABASE_URL}/rest/v1/profiles?id=eq.{user['id']}&select=*",
        headers=supa_headers(token), timeout=10)
